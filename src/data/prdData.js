...
export const transferModules = [
  ...
  },
  {
    ...
    type: 'Transfer In',
    summary: '客户从信托账户转入美国账户用于后续汇款或账户显示。BaaS 账户实际可能没有资金，后台可通过 internal funding / OTC / manual completion 完成。客户不看到 USDT、OTC、BaaS 地址或链上记录。',
    flow: ['客户选择从信托账户转入美国账户', '客户输入转入金额', '系统生成 transfer-in 订单', '系统冻结信托账户资金', '客户看到“处理中”', 'Admin 确认信托账户资金', 'Admin 按需执行 internal funding / OTC', 'Admin 确认转入完成', '系统更新 client_available_balance 或 pending_transfer_in_balance', '客户看到“已完成 / 可用”'],
    clientVisible: ['转入金额', '来源：信托账户', '目标：美国账户', '状态：Processing / Completed / Failed', '完成后的美国账户可用余额变化'],
    clientHidden: ['USDT funding', 'tx hash', 'BaaS USDT address', 'OTC cost', 'OTC counterparty', 'Interlace actual balance', 'Interlace fee', 'Fidere margin'],
    adminVisible: ['trust account balance', 'transfer-in order', 'frozen amount', 'internal funding status', 'USDT amount', 'tx hash', 'OTC record', 'execution cost', 'margin', 'manual completion action', 'audit log'],
    status: ['TRANSFER_IN_SUBMITTED', 'TRUST_BALANCE_CHECK_PENDING', 'TRUST_BALANCE_FROZEN', 'INTERNAL_FUNDING_PENDING', 'INTERNAL_FUNDING_CONFIRMED', 'OTC_PENDING', 'OTC_COMPLETED', 'ADMIN_CONFIRM_PENDING', 'COMPLETED', 'FAILED', 'CANCELLED'],
...
export const transferSimulatorScenarios = {
  ...
  inB: {
    ...
    amount: 50000,
    steps: ['TRANSFER_IN_SUBMITTED', 'TRUST_BALANCE_FROZEN', 'INTERNAL_FUNDING_PENDING', 'OTC_COMPLETED', 'ADMIN_CONFIRM_PENDING', 'COMPLETED'],
    client: [
    ...
    admin: [
      ...
      ['Internal Funding', 'USDT 50,000 demo', '严禁客户可见'],
      ['OTC 成本', 'USD 120.00 demo', '严禁客户可见'],
    ...
    hidden: ['BaaS USDT address', 'Internal USDT Funding', 'tx hash', 'OTC cost', 'Fidere margin', 'Interlace actual balance'],
  },
  outA: {
    title: '转出 A：美国账户余额转出',
...
export const transferDataModels = [
  ...
  ['us_account_transfers', '记录美国账户转出订单', 'id、client_id、trust_id、us_account_id、beneficiary_id、beneficiary_bank_account_id、transfer_source_type、payout_currency、payout_amount、fidere_fee_amount、interlace_fee_amount、total_client_deduction、status、client_confirmed_at、reviewed_by、reviewed_at、completed_at、created_at', '部分可见', 'transfer_source_type: US_ACCOUNT_ACTUAL_BALANCE / TRUST_ACCOUNT_PREFUNDING。'],
